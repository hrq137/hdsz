...
<style>
		...
		#rq_tabs a span { float:left;padding:5px 15px 4px 37px;color:#696969; }
...
<div id="menu" class="menu" style="overflow:scroll;background: white; overflow-x: hidden; overflow-y: auto;">
	...
	<div style='padding-bottom:1px;text-align:left;display: inline-block;float: left;' >
		...
		<label id="tab_item2">
		<a href='#' id='all' style='display:block;' onclick="allReport()">
		<span title="所有报表" style='cursor:pointer;width:250px; padding-left:0;font-weight:700'>
		  所有报表
		</span>
		</a>
		</label>
	<div>
		<label id="tab_item1">
		<a href='#' id='hz1' style='display:none;' onclick="moduleClick('/report/reportJsp/showReport_tb.jsp?raq=/ganBuTongJi/ganBuTongJi_zx/hz1_zx.raq&tjDate=<%=tjDate%>&startDate=<%=startDate%>&endDate=<%=endDate%>&userID=<%=userID%>&col=<%=col%>&opCod=<%=opCod%>&rybm=<%=rybm%>&dept=<%=dept%>',this)">
...
<script>
	...
	$(function(){
		...
		src["hz1"] = "/report/reportJsp/showReport_tb.jsp?raq=/ganBuTongJi/ganBuTongJi_zx/hz1_zx.raq&tjDate=<%=tjDate%>&startDate=<%=startDate%>&endDate=<%=endDate%>&userID=<%=userID%>&col=<%=col%>&opCod=<%=opCod%>&rybm=<%=rybm%>&dept=<%=dept%>";
