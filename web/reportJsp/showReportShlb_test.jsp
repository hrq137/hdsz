...
<td><button onclick="tb()" class="tbBtn">统计</button></td>
<td><button onclick="hztb()" class="tbBtn">汇总</button></td>
<td><button onclick="save1()" class="tbBtn">驳回</button></td>
<td><button onclick="save()" class="tbBtn">审核通过</button></td>
...
<script language="javascript">
	...
	//驳回
	function save1(){  
		var cellName =  selcheck().values;	
		var cellName1 = cellName.replace(/^\s*|\s*$/g,"");
		var att = cellName1.split(" ");
		cellLen = att.length;
		//alert(cellLen);
		var cell1=document.getElementById("report1_L5").attributes.value.value;
		for(i=0;i<cellLen;i++){
			var a=att[i];
			var cell2=document.getElementById(a).attributes.value.value;
				//alert(cell2);
				if(cell2!='0'){
					var cell2=document.getElementById(a);
					//alert(cell2);
					cell2.attributes.value.value='3';
		 ...
		 for(i=0;i<cellLen;i++){
			...
			var cell2=document.getElementById(a).attributes.value.value;
				...
				if(cell2!='3'){
					...
					cell2.attributes.value.value='0';
	...
	function hzsh(){
		...
		var cellName =  selUser().values;	
		//alert(cellName);
		var endDate =  selUser().endDate;  //获取选中得endDate得值
		console.log(endDate);	
		var startDate =  selUser().startDate;  //获取选中得startDate得值
		console.log(startDate);	
		var  user = document.getElementById('report1_H3');   
		console.log('A---->'+user);	
		userid =user.attributes.value.value;  //当前用户得单位id
		...
		//console.log("zjj---->"+a);
			var b = a.replace('G','I');
			var acell2=document.getElementById(a).attributes.value.value;
			var I=document.getElementById(b).attributes.value.value;
			if(I !='0'){
				alert("审核通过后才能汇总！");
				break;
			}
		...
		var tjDate  = att[0].replace('G','M');  
							 var tab_data   = att[0].replace('G','P');
		//console.log("tjDate---->"+tjDate)
		var ce =  c.substring(10);    //当前单位得下属子单位id
		var e=document.getElementById(tjDate).attributes.value.value;      //获取选中的第一个tjDate
						 var tab=document.getElementById(tab_data).attributes.value.value;        //获取选中的第一个tab
		//console.log("hrq---->"+e)
		window.location.href="/report/reportJsp/showReportGroupck.jsp?rpg=/ganBuTongJi/ganBuTongJi_hzsh/hzshGroup.rpg&tjDate="+e+"&tableName=干部统计表"+"&userID="+ce+"&tab="+tab+"&endDate="+endDate+"&startDate="+startDate+"&userid="+userid;
