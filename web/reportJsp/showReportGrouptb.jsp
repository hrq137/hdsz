...
<script type="text/javascript" src="js/checkd1b.js"></script>
...
<%
	...
	String dept = request.getParameter("dept"); //11111111111111111111111
    ...
    String url1 = "reportJsp/showReportGrouptb.jsp?rpg=/ganBuTongJi/ganBuTongJi_zx/zxGroup.rpg&tjDate="+tjDate+"&startDate="+startDate+"&endDate="+endDate+"&userID="+userID+"&col="+col+"&tab="+tab+"&dept="+dept+"&opCod="+opCod;      //33333333333保存数据后自动跳转页面
...
<td><button onclick="save1()" class="tbBtn">统计</button></td>
...
<td><button  class="tbBtn">表间检查</button></td>
	...
	function save1(){
		...
		if(cell1 != null){
			...
			if (E1 == '2' || E1=='0')
			{
			    ...
			    window.open("showReportGroupzq.jsp?rpg=/ganBuTongJi/ganBuTongJi_tbzq/tbzqGroup.rpg&tjDate="+tjDate+"&startDate="+startDate+"&endDate="+endDate+"&userID="+userID+"&col="+col+"&tab="+tab+"&dept="+dept+"&opCod="+opCod,"_self");
...
//保存直接入库
 function save() {
	          ...
	          if (cell1=='2'||cell1=='0')
			   ...
			   else if(cell1 == '' || cell1 == null){
				   alert('您还未保存过数据，不可以直接在报表组中保存');
			   }
	           else {
...
//导出Excel方法    只有审核通过才可以导出excl
 function excl(){
	    var cell1 = document.getElementById(data[length-1]+"_E1").attributes.value.value;		//这里的E1是展现表的单元格E1
		//alert(cell1);
		if(cell1=='0'){ //当cell1=='0'时，审核通过之后，可以导出操作
			group_saveAsExcel();return false;
		}else{
